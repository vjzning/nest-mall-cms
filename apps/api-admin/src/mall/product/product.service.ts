...
export class ProductService {
  ...
  async update(id: number, updateProductDto: UpdateProductDto) {
    ...
    try {
      ...
      // 1. Update Product
      await queryRunner.manager.update(MallProductEntity, id, productData);
      ...
      if (skus) {
        await queryRunner.manager.delete(MallProductSkuEntity, { productId: id });
        const skuEntities = skus.map(sku =>
          this.skuRepo.create({
            ...sku,
            productId: id,
        ...
        for (let j = 0; j < skuCount; j++) {
          skus.push(
            this.skuRepo.create({
              productId: p.id,
              code: `${p.id}-${j + 1}`,
              specs: [{ key: 'Color', value: j === 0 ? 'Red' : 'Blue' }],
