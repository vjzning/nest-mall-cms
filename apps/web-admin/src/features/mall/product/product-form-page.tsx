...
import { Trash2, Plus, ArrowLeft, Save, Image as ImageIcon, X, ChevronDown } from 'lucide-react';
...
export default function ProductFormPage() {
        ...
        product.skus.forEach(sku => {
          if (Array.isArray(sku.specs)) {
            sku.specs.forEach((s: any) => {
              if (!aggregated[s.key]) aggregated[s.key] = new Set();
              aggregated[s.key].add(s.value);
            });
          }
        });
        const recoveredSpecs = Object.entries(aggregated).map(([name, values]) => ({
          name,
  ...
  useEffect(() => {
    if (isSingleProduct && skuFields.length === 0) {
      replaceSkus([{ code: 'default', specs: [], price: 0, stock: 0, marketPrice: 0 }]);
    }
  ...
  const generateSkus = () => {
    ...
    const combinations =
      specs.length === 1 ? valuesArrays[0].map(v => [v]) : cartesian(...valuesArrays);

    const newSkus: ProductSku[] = combinations.map((combo: string[]) => {
      const specObj = combo.map((val, idx) => ({ key: specs[idx].name, value: val }));
  ...
  return (
    <div className="flex flex-col gap-4 pb-10 mx-auto w-full max-w-5xl h-full">
      <div className="flex gap-4 items-center">
        <Button variant="ghost" size="icon" onClick={() => navigate({ to: '/mall/product' })}>
        ...
        <div className="flex gap-2 ml-auto">
          <Button variant="outline" onClick={() => navigate({ to: '/mall/product' })}>
            Cancel
          </Button>
          <Button onClick={handleSubmit(onSubmit)} disabled={mutation.isPending}>
            <Save className="mr-2 w-4 h-4" /> Save Product
          </Button>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid grid-cols-3 w-full">
          ...
          <TabsTrigger value="skus">2. Specifications & SKUs</TabsTrigger>
          <TabsTrigger value="detail">3. Details & Images</TabsTrigger>
        </TabsList>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid gap-2">
                ...
                {/* Reusing TreeSelect from TreeDataTable/TreeSelect component if available or using Select if flat */}
                <TreeSelect
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="grid gap-2">
                  ...
                  <div className="flex items-center mt-2 space-x-2">
                    ...
                    />
                    <span>{watch('status') === 1 ? 'On Shelf' : 'Off Shelf'}</span>
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          <div className="grid gap-6">
            <div className="flex justify-between items-center px-2">
              <div className="flex gap-3 items-center">
                ...
                <Switch checked={isSingleProduct} onCheckedChange={handleSingleProductToggle} />
              </div>
              {!isSingleProduct && (
                <span className="text-sm text-muted-foreground">
                  Define specs and generate SKUs
                </span>
              )}
            </div>
            <Card>
              <CardHeader>
                ...
                <CardDescription>Define product variants (e.g. Color, Size)</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                {!isSingleProduct && (
                  <>
                    {specs.map((spec, idx) => (
                      <div key={idx} className="relative p-4 rounded-lg border bg-muted/20">
                        <Button
                          variant="ghost"
                          size="icon"
                              ...
                              {spec.values.map((val, vIdx) => (
                                <Badge key={vIdx} variant="secondary" className="pr-1">
                                  ...
                                  <button
                                    ...
                                    onClick={() => removeSpecValue(idx, vIdx)}
                                  >
                                    ×
                                  </button>
                                </Badge>
                    ...
                    <Button variant="outline" onClick={addSpec} className="w-full border-dashed">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Set price and stock for each variant</CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
                            ...
                            <div className="hidden absolute left-0 top-full z-10 p-2 w-32 rounded border shadow-md group-hover:block bg-popover">
                              <Input
                                ...
                                onKeyDown={e => {
                                  if (e.key === 'Enter') {
                                    e.preventDefault();
                                    applyBatchPrice(Number(e.currentTarget.value));
                                    e.currentTarget.value = '';
                                  }
                                }}
                              />
                            ...
                            <div className="hidden absolute left-0 top-full z-10 p-2 w-32 rounded border shadow-md group-hover:block bg-popover">
                              <Input
                                ...
                                onKeyDown={e => {
                                  if (e.key === 'Enter') {
                                    e.preventDefault();
                                    applyBatchStock(Number(e.currentTarget.value));
                                    e.currentTarget.value = '';
                                  }
                                }}
                              />
                      ...
                      <TableRow key={field.id}>
                        <TableCell>
                          {Array.isArray(field.specs)
                            ? field.specs.map((s: any) => `${s.value}`).join(' / ')
                            : JSON.stringify(field.specs)}
                        </TableCell>
                        <TableCell>
                          <Input {...register(`skus.${index}.code`)} readOnly={isSingleProduct} />
                        </TableCell>
                        <TableCell>
                          <Input
                            type="number"
                            ...
                            {...register(`skus.${index}.price`, {
                              valueAsNumber: true,
                              required: isSingleProduct ? 'Required' : undefined,
                              min: 0,
                            })}
                          />
                          {errors?.skus?.[index]?.price && (
                            <div className="mt-1 text-xs text-destructive">
                              {errors.skus[index]?.price?.message || 'Invalid price'}
                            </div>
                          )}
                        </TableCell>
                        <TableCell>
                          ...
                          {errors?.skus?.[index]?.stock && (
                            <div className="mt-1 text-xs text-destructive">
                              {errors.skus[index]?.stock?.message || 'Invalid stock'}
                            </div>
                          )}
                        </TableCell>
                        <TableCell>
                          <Button
                            ...
                            onClick={() => replaceSkus(skuFields.filter((_, i) => i !== index))}
                            disabled={isSingleProduct && skuFields.length === 1}
                          >
                            <Trash2 className="w-4 h-4" />
                          </Button>
                        </TableCell>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid gap-2">
                  ...
                  >
                    {cover ? (
                      ...
                      <div className="flex flex-col items-center text-muted-foreground">
                        ...
                        <span className="text-xs">Select Cover</span>
                      </div>
                    )}
                  </div>
                  {cover && (
                ...
                <Input {...register('cover')} className="hidden" />
              </div>
              ...
              <div className="grid gap-2">
                <Label>Product Detail (HTML/Markdown)</Label>
                <Textarea
                  {...register('detail')}
