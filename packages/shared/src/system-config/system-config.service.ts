...
export class SystemConfigService implements OnModuleInit, OnModuleDestroy {
  ...
  private async initRedis() {
    try {
      ...
      // We assume the client is node-redis compatible (as per @keyv/redis usage)
      if (!client) {
        this.logger.warn('No Redis client available');
        return;
      }
      // Duplicate connections for independent Pub/Sub
      this.pubClient = (client as any).duplicate();
      this.subClient = (client as any).duplicate();
