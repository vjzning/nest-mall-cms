...
import { SystemConfigService } from '../../../apps/api-admin/src/system/config/system-config.service';
...
export class StorageService implements OnModuleInit {
        ...
        switch (driverName) {
            case 'aliyun-oss':
                const ossConfig: AliyunOssConfig = {
                    ...
                    accessKeyId:
                        this.configService.get('storage.oss.accessKeyId') || '',
                    accessKeySecret:
                        this.configService.get('storage.oss.accessKeySecret') ||
                        '',
                ...
                if (!ossConfig.accessKeyId || !ossConfig.bucket) {
                    this.logger.warn(
                        'Aliyun OSS config missing, falling back to local'
                    );
            ...
            case 'aws-s3':
                const s3Config: AwsS3Config = {
                    ...
                    accessKeyId:
                        this.configService.get('storage.s3.accessKeyId') || '',
                    secretAccessKey:
                        this.configService.get('storage.s3.secretAccessKey') ||
                        '',
                ...
                if (!s3Config.accessKeyId || !s3Config.bucket) {
                    this.logger.warn(
                        'AWS S3 config missing, falling back to local'
                    );
                    return new LocalStorageDriver();
                }
    ...
    private async refreshDriver() {
        const activeDriver =
            this.configService.get('storage.active_driver') || 'local';
    ...
    async remove(id: number): Promise<void> {
        ...
        try {
            ...
            this.logger.warn(
                `Could not instantiate driver ${resource.driver}, falling back to current driver`
            );
